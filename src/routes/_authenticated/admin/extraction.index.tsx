...
function ExtractionPage() {
  ...
  const jobs = useQuery({
    ...
    queryFn: async () => {
      const [rows] = await Promise.all([list(), recover({ data: { timeoutMinutes: 10 } })]);
      return rows;
    },
    refetchInterval: (q) => {
              ...
              {(jobs.data ?? []).map((j) => (
                <tr key={j.id}>
                  <td className="px-4 py-3">
                    ...
                    <div className="max-w-[360px] truncate text-xs text-muted-foreground">{j.original_filename}</div>
                  </td>
                  ...
                  <td className="px-4 py-3 text-right">
                    <Button asChild size="sm" variant="ghost">
                      <Link to="/admin/extraction/$jobId" params={{ jobId: j.id }}>Open →</Link>
                    </Button>
                  </td>
                </tr>
              ))}
